...
// Insertar comentario en la base de datos con el nombre del usuario
if (!empty($usr)) {
    $queryUsuario = "SELECT nombre FROM usuarios WHERE nombre = ?";
    $stmtUsuario = $conn->prepare($queryUsuario);
    $stmtUsuario->bind_param("s", $usr);
    $stmtUsuario->execute();
    $resultUsuario = $stmtUsuario->get_result();

    if ($resultUsuario->num_rows > 0) {
        $row = $resultUsuario->fetch_assoc();
        // Asegurémonos de que la columna 'nombre' existe en el resultado
        if (isset($row['nombre'])) {
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['comentario'])) {
                ...
                $queryInsertComentario = "INSERT INTO comentarios (nombre, comentario) VALUES (?, ?)";
                $stmtInsertComentario = $conn->prepare($queryInsertComentario);
                $stmtInsertComentario->bind_param("ss", $nombreUsuario, $comentario);

                if ($stmtInsertComentario->execute()) {
                    $ok = "Enviado con éxito";
                } else {
                    echo "Error al agregar el comentario: " . $stmtInsertComentario->error;
                }
            }
        } else {
            echo "Error: La columna 'nombre' no está definida en el resultado de la consulta 'usuarios'.";
        }
    } else {
        echo "Error: No se encontró el nombre de usuario en la tabla 'usuarios'.";
    }
}
